...
class OdooTalker:
    ...
    def get_last_order(self):
        ...
        self.last_order = self.models.execute_kw(db, self.uid, password,
            'x_zamowienie_fakturownia',
            'search_read',
            [],
            {
                'fields': ['x_name', 'x_numer', 'x_data_wystawienia', 'x_studio_status_1'],
                'limit': 1,
                'order': 'x_name desc'
             }
        )[0]
    ...
    def get_last_invoice(self):
        ...
        self.last_invoice = self.models.execute_kw(db, self.uid, password,
        ...
        print(f"Last invoice date: {self.last_invoice_date}, number: {self.last_invoice_num}")

    def get_estimates(self, estimates):
        last_index = next((index for index, estimate in enumerate(estimates) if estimate['fields']['x_numer']
                           == self.last_order_num), None)
        print(last_index)
    ...
    def get_invoices(self, invoices):
        ...
        last_day_invoices = self.models.execute_kw(db, self.uid, password,
            'x_faktura_sprzedazowa',
            'search_read',
    ...
    def upload_estimates(self):
        for estimate in self.estimates[::-1]:
            ...
            self.models.execute_kw(db, self.uid, password, 'x_zamowienie_fakturownia', 'create', [estimate['fields']])

            for position in estimate['positions']:
                print(f"Creating new position: {position}")
                self.models.execute_kw(db, self.uid, password, 'x_pozycja_zamowienia_fakturownia', 'create', [position])
    ...
    def upload_invoices(self):
        for invoice in self.invoices[::-1]:
            ...
            self.models.execute_kw(db, self.uid, password, 'x_faktura_sprzedazowa', 'create', [invoice['fields']])

            for position in invoice['positions']:
                print(f"Creating new position: {position}")
                self.models.execute_kw(db, self.uid, password, 'x_pozycja_faktury_sprzedazowej', 'create', [position])