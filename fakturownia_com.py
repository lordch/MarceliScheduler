...
class FaktTalker:
    ...
    def categorize_docs(self):
        ...
        self.estimates = [doc for doc in self.document_list if doc["kind"] == "estimate"]
    ...
    def prepare_estimates(self):
        ...
        for estimate in self.estimates:
            ...
            fields_dict = {key: estimate[value] for key, value in odoo_parameters.items()}
            fields_dict['x_wartosc_netto_pln'] = float(fields_dict['x_wartosc_netto_pln'])
            fields_dict['x_wartosc_netto_pln'] *= ex_rate
            num = estimate['number'].split("ZAM ")[1]
            positions = []
            for item in estimate['positions']:
                position_dict = {key: item[value] for key, value in estimate_position_parameters.items()}
    ...
    def prepare_invoices(self):
        ...
        for invoice in self.invoices:
            ...
            fields_dict = {key: invoice[value] for key, value in invoice_parameters.items()}
            ...
            for item in invoice['positions']:
                ...
                if "rozliczenie" not in position_dict['x_name'].lower() and "zaliczki" not in position_dict['x_name'].lower():
                    positions.append(position_dict)
